...
from services.daily_sync import sync_daily_market_pulse
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Starting Cloud Compare API...")

    # Validate critical config
    if not settings.has_gemini:
        logger.warning("⚠️  GEMINI_API_KEY not set — LLM features will fail!")
    if not settings.has_tavily:
        logger.info("ℹ️  TAVILY_API_KEY not set — market intel disabled (optional).")
    ...
    # Boot the dynamic macro background task
...
async def root():
    return {
        ...
        "endpoints": {
            "chat": "/api/chat",
            "blueprint": "/api/blueprint/{id}",
            "native_map": "/api/blueprint/map-native",
            "refine": "/api/chat/refine",
        },
    }
